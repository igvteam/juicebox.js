...
import {decodeQuery, extractQuery, extractConfig} from "../js/urlUtils.js";
...
suite("testURLs", function () {
    ...
    test("Session blob", async function () {
        ...
        const query = await extractQuery(url);
        ...
        const sessionJSON = await extractConfig(query);
    ...
    test("parameters", async function () {
        ...
        const config = await extractConfig(query);
